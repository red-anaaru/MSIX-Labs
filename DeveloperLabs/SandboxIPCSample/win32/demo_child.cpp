...
// Function to get and display the current process integrity level and AppContainer status
void DisplayProcessSecurityInfo()
{
  ...
  {
    dwLengthNeeded = 0;
    GetTokenInformation(hToken, TokenAppContainerSid, nullptr, 0, &dwLengthNeeded);

    TOKEN_APPCONTAINER_INFORMATION* pAppContainerInfo = (TOKEN_APPCONTAINER_INFORMATION*)LocalAlloc(0, dwLengthNeeded);
    if (pAppContainerInfo)
    {
      if (GetTokenInformation(hToken, TokenAppContainerSid, pAppContainerInfo, dwLengthNeeded, &dwLengthNeeded))
      {
        LPWSTR szAppContainerSid = nullptr;
        if (ConvertSidToStringSidW(pAppContainerInfo->TokenAppContainer, &szAppContainerSid))
        {
